...
export async function POST(request: NextRequest) {
  try {
    ...
    if (resolvedSettings.transport === "tcp" && !isTcpScaleTargetReachableOnRequest(request, resolvedSettings.host)) {
      return fail(
        "Yerel agdaki Ethernet teraziler bulut sunucudan test edilemez. Bu cihaz icin Bey360 masaustu/local kurulum kullanin veya teraziyi erisilebilir bir statik IP/VPN uzerinden baglayin.",
